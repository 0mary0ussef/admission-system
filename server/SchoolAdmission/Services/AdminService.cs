    ...
    public async Task<PaginationResponseDTO<dynamic>> GetStudentsForAdminPaginatedAsync(string adminEmail, PaginationRequestDTO request)
    {
        ...
        query = request.SortBy?.ToLower() switch
        {
            ...
            "finalyearscore" => request.SortOrder?.ToLower() == "desc"
                ? query.OrderByDescending(s => s.AdmissionProfile.ThirdPrepScore)
                : query.OrderBy(s => s.AdmissionProfile.ThirdPrepScore),
            "percentage" => request.SortOrder?.ToLower() == "desc"
                ? query.OrderByDescending(s => s.AdmissionProfile.MinistryExamPercentage)
                : query.OrderBy(s => s.AdmissionProfile.MinistryExamPercentage),
            "interviewscore" => request.SortOrder?.ToLower() == "desc"
                ? query.OrderByDescending(s => s.Id) // Will be sorted after data retrieval
                : query.OrderBy(s => s.Id), // Will be sorted after data retrieval
        ...
        // Apply interview score sorting if needed (since it's calculated)
    ...
    public async Task<PaginationResponseDTO<dynamic>> GetStudentsForSuperAdminPaginatedAsync(string superAdminEmail, PaginationRequestDTO request)
    {
        ...
        query = request.SortBy?.ToLower() switch
        {
            ...
            "percentage" => request.SortOrder?.ToLower() == "desc"
                ? query.OrderByDescending(s => s.AdmissionProfile.MinistryExamPercentage)
                : query.OrderBy(s => s.AdmissionProfile.MinistryExamPercentage),
            "totalscore" => request.SortOrder?.ToLower() == "desc"
                ? query.OrderByDescending(s => s.Id) // Will be sorted after data retrieval
                : query.OrderBy(s => s.Id), // Will be sorted after data retrieval
            _ => query.OrderBy(s => s.FullNameEn)
        };
        ...
        else if (request.SortBy?.ToLower() == "percentage")
        {
            result = request.SortOrder?.ToLower() == "desc"
