...
public class StudentService : IStudentService
{
    ...
    public async Task<bool> ValidateStudentForExamAsync(string nationalId)
    {
        ...
        return account?.Role?.RoleName == "PendingStudent";
    }
    ...
    public async Task UpdateStudentInfoAsync(string nationalId, StudentCompleteInfoDTO dto)
    {
        ...
        {
            account.AdmissionProfile.StudentName = dto.StudentNameEn ?? "";
            account.FullNameEn = dto.StudentNameEn ?? "";
            ...
            // Update the three boolean fields
            account.AdmissionProfile.HasOnlineTrainingCourses = dto.HasOnlineTrainingCourses;
            account.AdmissionProfile.HasIcdllicense = dto.HasICDLLicense;
            account.AdmissionProfile.HasLaptop = dto.HasLaptop;
    ...
    public async Task<List<object>> GetAllStudentsAsync()
    {
        return await _db.Accounts
            .Include(a => a.Role)
            .Where(a => a.Role.RoleName == "PendingStudent" && a.IsActive == true && a.Role.BusinessEntity == "Admission")
            .Select(a => new
            {
                id = a.Id,
