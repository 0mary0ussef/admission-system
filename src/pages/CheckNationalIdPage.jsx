...
const CheckNationalIdPage = () => {
  ...
  const checkNationalId = async () => {
    ...
    try {
      ...
      // Check if student already completed their info
      if (response.data.hasCompletedInfo) {
        setSuccess("معلوماتك مكتملة بالفعل! لا تحتاج لإكمالها مرة أخرى.");
        return; // Don't navigate, just show the message
      }

      // If we get here, the student exists and hasn't completed info
      setSuccess("تم العثور على الرقم القومي! جاري التوجيه لإكمال معلوماتك...");

      // Store the National ID and student data for the next step
